...
namespace lve {
	...
	void FirstApp::pickPhysicalDevice()
	{
	...
	void FirstApp::createLogicalDevice()
	{
	...
	void FirstApp::createGraphicsPipeline()
	{
		...
		if (fs::exists(vertPath)) {
			...
			std::cout << "vertsize:" << vertShaderCode.size() << std::endl;
			vertShaderModule = createShaderModule(vertShaderCode);
		}
		if (fs::exists(fragPath)) {
			auto fragShaderCode = readFile(fragPath.string());
			std::cout << "vertsize:" << fragShaderCode.size() << std::endl;
	...
	void FirstApp::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
	{
	...
	void FirstApp::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
	...
	VkImageView FirstApp::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
	{
	...
	void FirstApp::copyBuffer(VkBuffer srcBuffer, VkBuffer destBuffer, VkDeviceSize size)
	{
	...
	void FirstApp::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
	{
	...
	bool FirstApp::isDeviceSuitable(VkPhysicalDevice device)
	{
	...
	VkCommandBuffer FirstApp::beginSingleTimeCommands()
	{
		VkCommandBufferAllocateInfo allocInfo{};
		allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
		allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
	...
	void FirstApp::endSingleTimeCommands(VkCommandBuffer commandBuffer)
	{
	...
	void FirstApp::transitionImageLayout(VkImage image, VkFormat format,
	...
	QueueFamilyIndices FirstApp::findQueueFamilies(VkPhysicalDevice device)
	{
		...
		{
			//找到一个支持VK_QUEUE_GRAPHICS_BIT的队列族
	...
	bool FirstApp::checkDeviceExtensionSupport(VkPhysicalDevice device)
	{
	...
	SwapChainSupportDetails FirstApp::querySwapChainSupport(VkPhysicalDevice device)
	{
	...
	VkSurfaceFormatKHR FirstApp::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
	{
	...
	VkPresentModeKHR FirstApp::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> availablePresentModes)
	{
		VkPresentModeKHR bestMode = VK_PRESENT_MODE_FIFO_KHR;
		//三倍缓冲避免了撕裂现象,检测是否支持三倍缓冲
	...
	VkExtent2D FirstApp::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
	{
